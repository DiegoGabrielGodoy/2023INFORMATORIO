#Escribe un programa que pida al usuario un número y luego imprima si ese número es un número perfecto o no. 
#Un número perfecto es aquel que es igual a la suma de sus divisores propios (excluyendo el propio número).
#Los números perfectos son aquellos iguales a la suma de sus divisores: 6 se
#puede dividir por 1, 2 y 3, y cuando sumas esos números, el resultado es 6

numero = int(input("Ingrese un número, para conocer si es perfecto o no: "))

suma_divisores = 0 
for num in range(1, numero):
    if numero % num == 0:
        suma_divisores += num

if suma_divisores == numero:
    print(f"{numero} es un número perfecto")
else:
    print(f"{numero} no es un número perfecto")

